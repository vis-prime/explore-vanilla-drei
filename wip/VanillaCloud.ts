...
export class Clouds extends Group {
  ...
  onFrame: (state: any, delta: any) => void
  clouds: CloudState[]
  updateCloudsDrawRange: () => void
  ...
  constructor({ limit = 200, range, material = MeshLambertMaterial, texture, frustumCulled = true }: CloudsProps = {}) {
    ...
    const getCloudArray = () => {
      ...
      for (let index = 0; index < this.ref.children.length; index++) {
        const mesh = this.ref.children[index] as SingleCloud
      ...
      this.updateCloudsDrawRange()

      console.log('CLOUD COUNT CHANGED, UPDATING ARRAY')

      return clouds
    }

    this.updateCloudsDrawRange = () => {
      console.log('CLOUD DRAW RANGE UPDATED', clouds.length)
...
let cloudCount = 0
/* @__PURE__ */
export class SingleCloud extends Group {
  seed: number
  segments: number
  bounds: Vector3
  concentrate: string
  ...
  updateCloudStateArray() {
    ...
    console.log(this.cloudStateArray)

    // this.cloudStateArray = [...new Array(segments)].map(
    //   (_, index) =>
    //     ({
    //       segments,
    //       bounds: new Vector3(1, 1, 1),
  ...
  updateClouds() {
    ...
    // console.log('updateClouds', this.name)
