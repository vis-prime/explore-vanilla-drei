...
const GridMaterial = shaderMaterial<GridMaterialType & any>(
  {
    cellSize: 0.5,
    sectionSize: 1,
    fadeDistance: 100,
...
export const Grid = ({
  ...
  const ref = new THREE.Mesh(planeGeometry, gridMaterial)
  ...
  const update = (camera: THREE.Camera) => {
    plane.setFromNormalAndCoplanarPoint(upVector, zeroVector).applyMatrix4(ref.matrixWorld)
